...
(function ensurePlaceholders(){
  try {
      ...
      debugOverlay.innerHTML = `
        <div id="debugHeader">
          ...
          <div id="debugControls">
            <button id="dbgClear" class="dbg-btn">مسح</button>
            <button id="dbgCopy" class="dbg-btn">نسخ</button>
            <button id="dbgToggleSize" class="dbg-btn">🔽</button>
          </div>
        </div>
        <div id="debugSummary" style="padding:8px;color:#9bb0c8">0 سجلات</div>
    ...
    if(!document.getElementById('btnGen')) {
      var b = document.createElement('button'); b.id='btnGen'; b.textContent='⚡ توليد G-code'; b.className='primary';
    ...
    if(!document.getElementById('btnContour')) {
      var b = document.createElement('button'); b.id='btnContour'; b.textContent='🌀 توليد G-code (Contour)'; b.className='secondary';
      var parent = document.getElementById('routerSettings') || document.getElementById('rightPanel') || document.body;
      parent.appendChild(b);
    }
    if(!document.getElementById('btnLaserEngrave')) {
      var b = document.createElement('button'); b.id='btnLaserEngrave'; b.textContent='توليد كود ليزر'; b.className='primary';
...
/* ======= Compatibility shim appended to ensure key inits run after DOM ready ======= */
(function(){
  function safeCall(fnName){
    try{
      ...
      if(typeof fn === 'function') {
        fn();
        console.log('safeCall: called', fnName);
        return true;
      }
    }catch(e){
      console.warn('safeCall error calling', fnName, e);
    }
    return false;
  }

  function attachIfMissing(id, tag, props, parent){
    if(!document.getElementById(id)){
      ...
      console.log('Compatibility shim: created element', id);
      return el;
    }
    return document.getElementById(id);
  }

  function ensureListener(btnId, handlerName){
    var btn = document.getElementById(btnId);
    if(!btn){
      btn = attachIfMissing(btnId, 'button', {textContent:btnId});
    }
    var has = false;
    // naive detection: check outerHTML for handlerName or listeners marker
    try{
      if(btn.onclick) has = true;
    }catch(e){}
    if(!has){
      btn.addEventListener('click', function(ev){
        ev.preventDefault();
        if(typeof window[handlerName] === 'function'){
          try{ window[handlerName](); }
          catch(e){ console.error('Error running handler', handlerName, e); }
        } else {
          console.warn('Handler', handlerName, 'not found.');
        }
      });
      console.log('Compatibility shim: attached listener for', btnId, '->', handlerName);
    }
  }
  ...
  whenReady(function(){
    // Ensure debug overlay exists and init runs
    attachIfMissing('debugOverlay','div', {innerHTML:'', className:'debug-overlay-fallback'});
    safeCall('initDebugOverlay');

    // Ensure canvases exist
    attachIfMissing('canvasOriginal','canvas');
    attachIfMissing('canvasHeatmap','canvas');
    attachIfMissing('canvasContour','canvas');
    attachIfMissing('canvas3D','canvas');

    // Ensure buttons/listeners
    ensureListener('btnGen','generateRasterGcode');
    ensureListener('btnContour','generateContourGcode');
    ensureListener('btnLaserEngrave','generateLaserGcode');

    // If OpenCV loads late, try to call onRuntimeInitialized hooks
    try{
      if(window.cv && !window.cv.calledCompatibilityShim){
        if(typeof window.cv['onRuntimeInitialized'] === 'function'){
          var orig = window.cv.onRuntimeInitialized;
          window.cv.onRuntimeInitialized = function(){
            try{ if(typeof orig === 'function') orig(); }catch(e){}
            try{ safeCall('onCvReady'); }catch(e){}
          };
        } else {
          // mark and call onCvReady if exists
          setTimeout(function(){ safeCall('onCvReady'); }, 1000);
        }
        window.cv.calledCompatibilityShim = true;
      }
    }catch(e){ /* ignore */ }

    console.log('Compatibility shim: finished initialization.');
  });
