...
/* ======= Enhanced Debug & Safe G-code Generation (Arabic messages) ======= */
(function(){
  function ensureLogDebug(){
    if(typeof window.logDebug === 'function') return;
    window.logDebug = function(msg, type){
      try {
        var debugList = document.getElementById('debugList');
        if(!debugList){
          var debugOverlay = document.getElementById('debugOverlay') || document.createElement('div');
          debugOverlay.id = 'debugOverlay';
          document.body.appendChild(debugOverlay);
          debugList = document.createElement('div');
          debugList.id = 'debugList';
          debugOverlay.appendChild(debugList);
        }
        var entry = document.createElement('div');
        entry.className = 'dbg-item ' + (type || 'dbg-info');
        ...
        debugList.prepend(entry);
        while(debugList.childElementCount > 200) debugList.removeChild(debugList.lastChild);
      } catch(e){
        console.log('logDebug fallback error', e);
      }
    };
  }
  ensureLogDebug();
  (function(){
    ...
    console.log = function(){
      try{ window.logDebug(Array.from(arguments).join(' '), 'dbg-info'); }catch(e){}
      _log.apply(console, arguments);
    };
    console.warn = function(){
      try{ window.logDebug(Array.from(arguments).join(' '), 'dbg-warn'); }catch(e){}
      _warn.apply(console, arguments);
    };
    console.error = function(){
      try{ window.logDebug(Array.from(arguments).join(' '), 'dbg-error'); }catch(e){}
  ...
  window.addEventListener('error', function(ev){
    try{
      ...
      window.logDebug('Unhandled Error: ' + msg + ' ' + src, 'dbg-error');
    }catch(e){}
  });
  window.addEventListener('unhandledrejection', function(ev){
    try{
      var reason = ev && ev.reason ? ev.reason : ev;
      window.logDebug('Unhandled Promise Rejection: ' + (reason && reason.message ? reason.message : String(reason)), 'dbg-error');
    }catch(e){}
  });
  window.safeGenerate = function(fnName){
    try {
      if(typeof window[fnName] === 'function'){
        window.logDebug('تشغيل الدالة: ' + fnName, 'dbg-info');
        var res = window[fnName]();
        window.logDebug('تم تنفيذ ' + fnName, 'dbg-info');
        return res;
      } else {
        throw new Error('الدالة ' + fnName + ' غير معرفة');
      }
    } catch (e) {
      console.error('خطأ أثناء ' + fnName + ':', e);
      try {
        window.logDebug('❌ خطأ أثناء ' + fnName + ': ' + (e && e.message ? e.message : String(e)), 'dbg-error');
      }catch(err){}
      try { if(typeof showToast === 'function') showToast('فشل في تنفيذ: ' + fnName); }catch(e){}
      return null;
    }
  };
  function bindSafeButtons(){
    try {
      var btns = [
        {id:'btnGen', fn:'generateRasterGcode'},
        {id:'btnContour', fn:'generateContourGcode'},
        {id:'btnLaserEngrave', fn:'generateLaserGcode'},
        {id:'btnQuick', fn:'generateRasterGcode'},
        {id:'btnLaserQuick', fn:'generateLaserGcode'}
      ];
      btns.forEach(function(b){
        var el = document.getElementById(b.id);
        if(el && !el.__safeBound){
          el.addEventListener('click', function(ev){
            ev.preventDefault();
            window.safeGenerate(b.fn);
          });
          el.__safeBound = true;
          console.log('ربط الزر ' + b.id + ' بـ ' + b.fn);
        }
      });
    } catch(e){
      console.error('bindSafeButtons error', e);
    }
  }
  ...
  whenReady(function(){
    try{
      bindSafeButtons();
      if(typeof window.initDebugOverlay === 'function') {
        try{ window.initDebugOverlay(); console.log('initDebugOverlay called by enhancer'); }catch(e){}
      }
    }catch(e){ console.error(e); }
  });
  window._ensureDummyGcode = function(){
    try{
      if(document.getElementById('gcodeOut') && (document.getElementById('gcodeOut').value||'').trim()===''){
        var txt = [];
        txt.push('; Dummy G-code generated for testing');
        txt.push('G21 ; mm');
        txt.push('G90 ; absolute');
        txt.push('G1 X0 Y0 Z5 F1000');
        txt.push('G1 X10 Y0 Z-1 F800');
        txt.push('G1 X10 Y10 Z-1 F800');
        txt.push('G1 X0 Y10 Z-1 F800');
        txt.push('G1 X0 Y0 Z5 F1000');
        txt.push('M2');
        document.getElementById('gcodeOut').value = txt.join('\\n');
        window.logDebug('تم إنشاء G-code اختباري افتراضي.', 'dbg-info');
      }
    }catch(e){ console.error(e); }
  };
})();
